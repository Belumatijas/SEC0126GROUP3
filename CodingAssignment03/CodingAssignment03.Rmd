...
model_transformation1 <- lm(LnCharges ~., data = training_set[,c(10,11,4,6:9)])
...
model_transformation2 <- lm(LnCharges ~., data = training_set[,c(10,2,4,6:9)])
...
## [1] 0.07170367
```
Looking at these two models, the second model was the worst performing. Looking at the other one, the logarithmic relationship has higher bias, lower MAE, and lower MAPE. A MAPE less than 5%, which Model 1 has, means that a prediction is acceptably accurate. Model 1 has a lower RMSE meaning that there were not large prediction errors. If you were to pick between one model or the other, it would depend on your time preference. If you are looking to choose in the short-run, then Model 1. If the long-run, then Model 2. 
